...
The Zen `config` object can have the following properties:
...
* If you use a `.zenrc.js` file, then add the platform to `config.builders.{platform}.stack` where `{platform}` must be
